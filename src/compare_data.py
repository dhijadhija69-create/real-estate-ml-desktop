...
def compare_data():
    ...
    print(
        f"Raw Data:       {
            ...
            raw_data.shape[1]} columns")
    print(
        f"Processed Data: {
            processed_data.shape[0]} rows × {
            processed_data.shape[1]} columns")
    rows_removed = raw_data.shape[0] - processed_data.shape[0]
    print(
        f"Rows Removed:   {rows_removed} rows ({(rows_removed / raw_data.shape[0] * 100):.2f}%)")
    ...
    if raw_missing.sum() > 0:
        for col in raw_missing[raw_missing > 0].index:
            print(
                f"  - {col}: {raw_missing[col]} ({(raw_missing[col] / len(raw_data) * 100):.2f}%)")
    else:
    ...
    if processed_missing.sum() > 0:
        for col in processed_missing[processed_missing > 0].index:
            print(
                f"  - {col}: {processed_missing[col]} ({(processed_missing[col] / len(processed_data) * 100):.2f}%)")
    else:
        print("  No missing values")
    ...
    print(f"✓ Duplicates Removed: {duplicates_removed}")
    if area_col and price_col:
        invalid_removed = raw_invalid_area + raw_invalid_price - \
            processed_invalid_area - processed_invalid_price
        print(
            f"✓ Invalid Values Removed: {invalid_removed} (Area ≤ 0 or Price ≤ 0)")
